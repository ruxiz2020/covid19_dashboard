...
from controls import COUNTIES, METRIC_TYPES, WELL_TYPES, WELL_COLORS
...
def filter_dataframe(df, metric_types, well_types, year_slider):
    dff = df[
        df["metric_type"].isin(metric_types)
...
@app.callback(
    ...
    [
        Input("metric_types", "value"),
        Input("well_types", "value"),
        Input("year_slider", "value"),
    ],
)
def update_production_text(metric_types, well_types, year_slider):

    dff = filter_dataframe(df, metric_types, well_types, year_slider)
...
# Radio -> multi
@app.callback(
    Output("metric_types", "value"), [Input("metric_type_selector", "value")]
)
def display_status(selector):
    if selector == "all":
        return list(METRIC_TYPES.keys())
...
def update_well_text(metric_types, well_types, year_slider):
...
def make_main_figure(
    metric_types, well_types, year_slider, selector, main_graph_layout
):
...
def make_aggregate_figure(metric_types, well_types, year_slider, main_graph_hover):
...
def make_pie_figure(metric_types, well_types, year_slider):
...
def make_count_figure(metric_types, well_types, year_slider):

    layout_count = copy.deepcopy(layout)

    dff = filter_dataframe(df, metric_types, well_types, [1960, 2017])
    g = dff[["API_WellNo", "date"]]
    g.index = g["date"]
